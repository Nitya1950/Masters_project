...
if __name__ == "__main__":
    ...
    @dataclasses.dataclass(slots=True)
    class GlobalConfig:
        phasmoconfig: gtaactions.Config
        twitchconfig: Config
    ...
    myphasmoconfig = gtaactions.Config(config={
    ...
    def get_phasmo_config() -> gtaactions.Config:
        return global_config.phasmoconfig

    def get_twitch_config() -> Config:
        return global_config.twitchconfig

    myactions = {key: TwitchAction(get_twitch_config, key, value) for key, value in gtaactions.all_actions_dict(get_phasmo_config).items()}
