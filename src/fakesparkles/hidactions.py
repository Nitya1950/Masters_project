...
def start_listeners(on_press: KeybindChangeCallback) -> tuple[KeyboardListener, MouseListener]:
    ...
    Args:
        on_press (KeybindChangeCallback): The function to call when a key or button is pressed.

    Returns:
        tuple[KeyboardListener, MouseListener]: A tuple containing the KeyboardListener and MouseListener objects.

    """
    global keyboard_listener, mouse_listener

    print("Starting keyboard and mouse listeners...", end="", flush=True)  # We flush so it is very clear when we start keylogging
...
def test_run(message: str) -> errorcodes.ErrorSet:
    """Testing each input"""
    if DEBUG:
        print(message)
...
class PressButton:
    ...
    def run(self, *, force: bool = False) -> errorcodes.ErrorSet:
        ...
        return test_run(f"Pressing button: {self.config.button}")
...
class ReleaseButton:
    ...
    def run(self, *, force: bool = False) -> errorcodes.ErrorSet:
        ...
        return test_run(f"Releasing button: {self.config.button}")
...
class MoveMouse:
    ...
    def run(self, *, force: bool = False) -> errorcodes.ErrorSet:
        """Run the action"""
        return test_run(f"Moving mouse to: {self.config.x}, {self.config.y}")
...
class MoveMouseRelative:
    ...
    def run(self, *, force: bool = False) -> errorcodes.ErrorSet:
        ...
        return test_run(f"Moving mouse (relative) by: {self.config.x}, {self.config.y}")
...
class MoveMouseRelativeSmooth:
    ...
    def run(self, *, force: bool = False) -> errorcodes.ErrorSet:
        ...
        return test_run(f"Moving mouse smoothly (relative) by: {self.config.x}, {self.config.y} in {self.config.repeats} repeats with {self.config.pause} pause")
...
class MoveMouseRelativeDirection:
    ...
    def run(self, *, force: bool = False) -> errorcodes.ErrorSet:
        ...
        return test_run(f"Moving mouse (relative direction) by: {self.config.distance} in direction {self.config.direction.value}")
...
class MoveMouseRelativeDirectionSmooth:
    ...
    def run(self, *, force: bool = False) -> errorcodes.ErrorSet:
        ...
        if not DEBUG:
            ...
            return actions.ActionRepeatWithWait(look_action, self.config.repeats, actions.Wait(self.config.pause)).run(force=force)
        return test_run(f"Moving mouse smoothly (relative direction) by: {self.config.distance} in directino {self.config.direction.value} repeats with {self.config.pause} pause")
...
class PressKey:
    ...
    def run(self, *, force: bool = False) -> errorcodes.ErrorSet:
        ...
        return test_run(f"Pressing key: {self.config.key}")
...
class ReleaseKey:
    ...
    def run(self, *, force: bool = False) -> errorcodes.ErrorSet:
        ...
        return test_run(f"Releasing key: {self.config.key}")
