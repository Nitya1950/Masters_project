...
DEFAULT_CHANNELS = {"fakesparkles"}
DEFAULT_SUPERUSERS = {"fakesparkles"}
...
@dataclasses.dataclass(slots=True)
class Config:
    """The global config for Twitch_Chatplay"""
    config: ConfigDict
    version: str
    enabled: bool = True
    ...
    def load(cls, version: str, filename: str = None) -> Self:
        ...
        gta = gameactions.Config.from_toml(phasmotoml, default=gtaactions.default_config())
        twitch = twitchactions.from_toml(twitchtoml, gta.config.keys())

        assert all(key in twitch.config for key in gta.config.keys())
    ...
    def _make_superuser_actions(self, twitch_actions: twitchactions.ActionDict) -> dict[str, Callable]:
        ...
        return {
            self.superuser_prefix + " reload": lambda: print("Reload!"),
