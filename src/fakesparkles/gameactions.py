...
class Config:
    ...
    def from_toml(cls, existing: dict[str, dict[str, Any]], *, default: Self | None = None) -> Self:
        ...
        ret: Config = default or Config()
        for key in ret.config.keys():
            if key in existing:
                to_replace: ActionConfig = ret.config[key]
                using_this: dict[str, Any] = existing.get(key, None)
                if using_this:
                    to_replace_hidconfig: hidactions.Config = to_replace.hidconfig
                    using_this_hidconfig: dict[str, Any] = using_this.get("hidconfig", None)
                    kwargs: dict[str, Any] = {**using_this}
                    if to_replace_hidconfig:
                        hiddevice: hidactions.HidType = hidactions.HidType[kwargs["hidconfig"]["device"]] if "device" in kwargs["hidconfig"] else to_replace_hidconfig.device
...
class GenericActionBase:
    """Generic gta action base class"""
    config_fn: ConfigFn

    @property
    def config(self) -> ActionConfig | None:
        """Get the config for this action"""
        return self.config_fn()[self.name]
