...
def make_window(cfg: config.Config, width_px: int, height_px: int) -> tk.Tk:
    """Making the window"""
    window = tk.Tk()
    window.title("Twitch Chat Plays!")
...
def enabled_cb(cfg: config.Config, enabled_button: tk.Button, state_var: tk.BooleanVar) -> None:
    ...
    if state_var.get():
        ...
        enabled_button.configure(text="Disabled", fg="black", bg="indian red")
        cfg.enabled = False
        state_var.set(False)
        print("Disabled")
    else:
        # Currently off, turn on
        enabled_button.configure(text="Enabled", fg="white", bg="sea green")
        cfg.enabled = True
        state_var.set(True)
        print("Enabled")
...
def make_gui(cfg: config.Config) -> tuple[tk.Tk, mp.Event, Callable]:
    """Make the GUI"""
    canvas = make_canvas(cfg, environment.resource_path("assets", "glitter_640.png"))
